...
fun EditarDocente(
    ...
    Scaffold(
        topBar = {
            ...
            TopAppBar(
                title = { Text("Actualizar Docente", fontWeight = FontWeight.Bold) },
            ...
            Column {
                Button(
                    onClick = {
                        ...
                        imageCaptured = false
                        val uri = createImageUri(context, "docente_${System.currentTimeMillis()}.jpg")
                        imageUri = uri
                        launcher.launch(uri)
                    }
                ) {
                ...
                val imagenAMostrar: Any = if (imageCaptured && imageUri != null) {
                    // El query param "?t=" hace que Coil crea que es una imagen nueva y no use el caché
                    ...
                    scope.launch {
                        try {
                            if (nombres.isNotBlank() && apellidos.isNotBlank() && nomSexo.isNotBlank() && sueldo.isNotBlank() && hijos.isNotBlank()) {
                                var urlImagen = foto // Mantener la foto actual por defecto
                                ...
                                SnackbarManager.showMessage("Docente actualizado con éxito")
                                onBack()
                            } else {
                                SnackbarManager.showMessage("Por favor complete todos los campos")
                            }
                        } catch (e: Exception) {
                            SnackbarManager.showMessage("Error al subir/guardar: ${e.message}")
                            // Log para debug
                            println("DEBUG_UPLOAD_ERROR: ${e.message}")
                        }
                    }
                },
                modifier = Modifier.fillMaxWidth(),
